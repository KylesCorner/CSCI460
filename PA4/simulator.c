...
int pageout(int process, int page) { 
    if (process<0 || // process must be a postive integer
        process>=procs || // process must be within bounds of MAXPROCESS
        !processes[process] || // process must be within the current process array
        !processes[process]->active ||// process must be active
        page<0 ||// page must be a positive integer.
        page>=processes[process]->npages) // page number must be less than the number of pages
	return FALSE; 

    if (processes[process]->pages[page]<0) //if page is avalable(-1) to be swapped
	return TRUE; /* on its way out */ 

    if (processes[process]->pages[page]>0) // page table entry for process page is true(1)
...
int pagein(int process, int page) { 
    if (process<0 || // Process must be a positive integer
        process>=procs || // process must be within bounds of MAXPROCESS
        !processes[process]|| // process must be within current process array.
        !processes[process]->active || //process must be active
        page<0 ||
