...
# Default for hiding duplicate and triplicate options with (upstart) and (recovery mode)?
...
# Major / Minor internal grub submenu numbers, i.e., `1>0`, `1>1`, `1>2`, etc.
ThisSubMenuMajorNo=0
NextSubMenuMinorNo=0

CurrTag=""    # Current grub internal menu number, zero-based
CurrText=""   # Current grub menu option text, i.e., "Ubuntu", "Windows...", etc.
...
while read -r line; do
    ...
    if [[ $line == submenu* ]]; then
        ...
        CurrText="${line#*\'}"
        CurrText="${CurrText%%\'*}"
        AllMenusArr+=("$CurrTag" "$CurrText") # i.e., "1 Advanced options for Ubuntu"
